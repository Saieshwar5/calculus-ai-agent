"""
Query API routes.
Handles streaming query endpoints with file upload support.
"""
...
from app.schemas.pydantic_schemas.memory_schema import MemoryResponse, MemoryClearResponse
from app.services.episodic_memory_trigger import get_episodic_memory_trigger
...
async def stream_query(
    ...
    try:
        ...
        if files:
            ...
            print(f"Received {len(file_names)} file(s): {file_names}")
        ...
        async def response_generator():
            async for chunk in stream_query_response(query_id, db, user_id, memory_manager, query_text.strip()):
                yield chunk
